...
from src.orders.schemas import OrderReadSchema, OrderItemCreateSchema
from src.orders.services import create_order_from_cart, mark_order_paid, get_order
...
async def create_order_endpoint(
    ...
    """
    try:
        order = await create_order_from_cart(db, user_id, [item.model_dump() for item in cart_items])
        return order
...
async def pay_order_endpoint(order_id: int, db: AsyncSession = Depends(get_db)):
    """
