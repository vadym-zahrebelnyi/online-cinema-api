...
@router.post("/", response_model=schemas.MovieDetailSchema, dependencies=[Depends(allow_moderator)])
...
@router.get("/{movie_id}", response_model=schemas.MovieDetailSchema)
...
@router.patch("/{movie_id}", response_model=schemas.MovieDetailSchema, dependencies=[Depends(allow_moderator)])
...
@router.delete("/genres/{genre_id}", dependencies=[Depends(allow_admin)])
async def delete_genre(genre_id: int, db: DbSession):
    await service.delete_genre(db, genre_id)
    return {"detail": "Genre deleted"}

@router.post("/certifications/", response_model=schemas.CertificationReadSchema, dependencies=[Depends(allow_moderator)])
...
@router.get("/certifications/", response_model=list[schemas.CertificationReadSchema])
async def list_certifications(db: DbSession):
    return await service.list_certifications(db)

@router.get("/certifications/{cert_id}", response_model=schemas.CertificationReadSchema)
...
@router.delete("/certifications/{cert_id}", dependencies=[Depends(allow_admin)])
