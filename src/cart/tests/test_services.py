...
from src.cart.services import CartService, MovieNotFoundError, MovieAlreadyOwnedError, MovieAlreadyInCartError
...
def mock_repo():
    ...
    class AsyncCM:
        ...
        async def __aexit__(self, exc_type, exc_val, exc_tb):
            return None
    repo.db.begin = MagicMock(return_value=AsyncCM())
...
async def test_add_movie_success(cart_service_instance, mock_repo, movie_a):
    ...
    mock_repo.add_item.assert_awaited_once()
...
async def test_remove_item(cart_service_instance, mock_repo, movie_a):
    ...
    mock_repo.remove_item.assert_awaited_once_with(movie_a.id, user_id)
...
async def test_clear_cart(cart_service_instance, mock_repo):
    ...
    mock_repo.clear_cart.assert_awaited_once()
...
async def test_add_movie_already_in_cart(cart_service_instance, mock_repo, movie_a):
    ...
    mock_repo.get_movie.return_value = movie_a
    mock_repo.is_movie_available_to_buy.return_value = False
    mock_cart_item = MockCartItem(id=101, movie=movie_a, added_at=datetime.utcnow())
